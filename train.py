...
def train(opt):
    # training epoch loop
    train_losses = []  # list untuk menyimpan loss pada setiap epoch
    valid_accs = []  # list untuk menyimpan akurasi pada setiap epoch
    ...
    for ep in range(opt.num_epoch):
        ...
        if ep % opt.valid_interval == 0:
            ...
            # simpan loss dan akurasi pada list
            train_losses.append(avg_loss / len(trainloader))
    ...
    plt.subplot(1, 2, 1)
    plt.plot(range(1, opt.num_epoch + 1), train_losses, label='Training Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()

    plt.subplot(1, 2, 2)
    plt.plot(range(1, opt.num_epoch + 1), valid_accs, label='Validation Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
